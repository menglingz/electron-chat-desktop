...
export const reqRegister = (userInfo: UserInfoData) =>
  request<any, RegisterResponse>({ url: API.REGISTER_URL, method: 'post', data: queryString.stringify(userInfo) });
// 登录接口
export const reqLogin = (userInfo: UserInfoData) =>
  request<any, LoginResponse>({ url: API.LOGIN_URL, method: 'post', data: queryString.stringify(userInfo) });
